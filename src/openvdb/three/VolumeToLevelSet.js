...
export class VolumeToLevelSet extends Three.Group {
  processes = [];

  constructor(source, material, { resolution, progressive }, onConverted, onProgress) {
    super();
    ...
    grids.forEach((grid, gridIndex) => {
      ...
      function* probeValue() {
        for (let i = 0; i < resolutionPow3; i++) {
          ...
          // Index 0
          referencePoint.copy(target);
          values[0] = grid.getValue(referencePoint);

          // Index 1
          referencePoint.copy(target);
          referencePoint.x += step.x;
          values[1] = grid.getValue(referencePoint);

          // Index 2
          referencePoint.copy(target);
          referencePoint.x += step.x;
          referencePoint.z += step.z;
          values[2] = grid.getValue(referencePoint);

          // Index 3
          referencePoint.copy(target);
          referencePoint.z += step.z;
          values[3] = grid.getValue(referencePoint);

          // Index 4
          referencePoint.copy(target);
          referencePoint.y += step.y;
          values[4] = grid.getValue(referencePoint);

          // Index 5
          referencePoint.copy(target);
          referencePoint.x += step.x;
          referencePoint.y += step.y;
          values[5] = grid.getValue(referencePoint);
          ...
          referencePoint.y += step.y;
          referencePoint.z += step.z;
          values[6] = grid.getValue(referencePoint);

          // Index 7
          referencePoint.copy(target);
          referencePoint.y += step.y;
          referencePoint.z += step.z;
          values[7] = grid.getValue(referencePoint);

          let cubeIndex = 0;

          if (values[0]) cubeIndex |= 1; // NOTE Put in binary mask
          ...
          const edgesDefinition = edgeLUT[cubeIndex];
          const trianglesDefinition = triangleLUT[cubeIndex] || trianglesDefinition[1];
          const triangleAverages = [
            [0.5, 0.0, 0.0],
            [1.0, 0.0, 0.5],
            [0.5, 0.0, 1.0],
            [0.0, 0.0, 0.5],
          ...
          if (values.filter((i) => i !== 0.0).length) {
            const color = { r: Math.random(), g: Math.random(), b: Math.random() };

            trianglesDefinition.forEach((triangleIndex) => {
              const triangle = triangleAverages[triangleIndex];

              vertices.push(
                x - resolution / 2.0 + triangle[0],
                y - resolution / 2.0 + triangle[1],
